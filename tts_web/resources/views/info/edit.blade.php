...
<article class="content forms-page">
    <div class="card">
        <h1 class="text-IBM">Cập nhật thông tin: {{ $info->type }}</h1>
    </div>
    <div class="row sameheight-container">
        <div class="col-md-12">
            <div class="card card-block">
                <form action="{{ url("/mn_info/$info->id/edit") }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $info->title) }}" class="form-control" placeholder="Title" aria-describedby="helpId">
                    </div>
                    @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title_e" id="title_e" value="{{ old('title_e', $info->title_e) }}" class="form-control" placeholder="Title English" aria-describedby="helpId">
                    </div>
                    @error('title_e')
                        <div class="form-text text-danger mb-3">{{ $message }}</div>
                    @enderror
