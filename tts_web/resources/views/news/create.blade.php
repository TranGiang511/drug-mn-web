...
@section('content')
    @if(Auth::check())
        <!-- create new new -->
        <form method="POST" action="{{ url("/news/create") }}" enctype="multipart/form-data" id="form-news">
            @csrf
            <main id="tt-pageContent" class="bg-main">
                <div class="container">
                    <div class="title-block">
                        <h3 class="title"> 
                            Create A New Vietnamese Post <span class="sparkline bar" data-type="bar"></span>
                        </h3>
                    </div>
                    <div name="item">
                                ...
                                <div class="col-sm-10">
                                    @if (!isset($news->id))
                                        <label class="radio-inline">
                                            <input type="radio" name="lang" value="vi" checked> Vietnamese
                                        </label>
                                        <div class="form-text text-danger" id="lang_error"></div>
                                    @else
                                        <label class="radio-inline">
                                            <input type="radio" name="lang" value="vi" @if($news->lang == 'vi') checked @endif disabled> Vietnamese
                                        </label>
                                        <label class="radio-inline">
                                            <input type="radio" name="lang" value="en" @if($news->lang == 'en') checked @endif> English
                                        </label>
                                        <div class="form-text text-danger" id="lang_error"></div>
                                        <button type="submit" id="update_en" class="btn btn-primary">Update English</button>
                                    @endif
                                    @if ($errors->has('lang'))
